...
| 平台管理员 | 维护系统配置；可进入任意社团上下文进行全权管理与处置 |
...
- 不同社团数据严格隔离；
- 平台管理员可进入任意社团并拥有该社团全权权限，操作全部记入对应社团审计日志（2026-09 需求变更，原“不默认查看所有发票正文”约束废止）；
- 权限在服务端校验，不能仅依赖页面隐藏；
- 用户离任后权限失效，历史操作署名保留。
...
- 支持保存常用筛选；
- 展示票面金额、申请金额和项目合计；
- 支持查看单张发票完整时间线：MVP 由服务端在重新校验数据权限后，聚合同一张发票的文件修订、审核记录、纸票事件、扫码事件、预检历史、审计操作和平台外状态事件；
- 支持权限范围内导出列表。
...
- 平台管理员进入社团上下文的全权操作写入对应社团审计日志（2026-09 需求变更，原“不默认拥有全部发票正文权限”约束废止）；
- 查看、下载和导出敏感数据写入日志；
- 数据库、文件和备份加密；
- 上传文件进行类型、大小和恶意内容检查；
