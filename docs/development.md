...
平台管理员除 `/api/platform/**` 平台级入口外，还可携带任意社团的 `X-Organization-Id` 请求头进入该社团上下文，拥有社团内全权管理能力（查看和审批发票、管理项目/成员/角色、台账、审计日志等），无需事先加入该社团；操作会以平台管理员身份记入该社团审计日志。停用的平台管理员（`status != ACTIVE`）不再享有该旁路。
